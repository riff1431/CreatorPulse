...
export class CloudflareR2StorageDriver extends BaseStorageDriver {
  readonly name: StorageDriverType = 'r2';
  ...
  public isConfigured(): boolean {
    return Boolean(
      ...
      (this.config.accountId || this.config.endpoint)
    );
  }
  ...
  private getR2Client(): S3Client {
    ...
    if (!this.client) {
      throw new Error('Cloudflare R2 driver is not fully configured (missing bucket, accountId/endpoint, accessKeyId, or secretAccessKey).');
    }
    return this.client;
  }
  ...
  public getPublicUrl(relativePath: string): string {
    ...
    // 1. Custom Public URL / R2 Dev URL
    if (this.config.publicUrl) {
      const base = this.config.publicUrl.endsWith('/') ? this.config.publicUrl.slice(0, -1) : this.config.publicUrl;
      return `${base}/${cleanPath}`;
    }

    // 2. Default R2 Endpoint fallback
    const endpoint = this.getEndpoint();
    if (endpoint) {
      const base = endpoint.endsWith('/') ? endpoint.slice(0, -1) : endpoint;
      return `${base}/${this.config.bucket}/${cleanPath}`;
    }
  ...
  public async download(relativePath: string): Promise<{ buffer: Buffer; mimeType: string }> {
    ...
    if (!response.Body) {
      throw new Error(`R2 GetObject returned empty body for key: ${cleanPath}`);
    }
  ...
  public async delete(relativePath: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`R2 delete failed for ${cleanPath}:`, err);
      return false;
    }
  }
  ...
  public async listFiles(folder?: StorageCategoryFolder): Promise<StoredFile[]> {
    ...
    if (response.Contents) {
      for (const item of response.Contents) {
        ...
        files.push({
          id: `r2_${item.ETag?.replace(/"/g, '') || item.Key}`,
          name: fileName,
  ...
  public async testConnection(): Promise<StorageTestResult> {
    ...
    if (!this.isConfigured()) {
      return {
        ...
        success: false,
        latencyMs: 0,
        message: 'Cloudflare R2 credentials, account ID, or bucket name are missing. Please complete the configuration.',
        readOk: false,
        writeOk: false,
        deleteOk: false,
    ...
    try {
      ...
      const testPayload = Buffer.from('creatorpulse-r2-test-ok', 'utf8');
      ...
      if (getRes.Body) {
        ...
        readOk = downloaded === 'creatorpulse-r2-test-ok';
      }
      ...
      return {
        ...
        latencyMs,
        message: `Cloudflare R2 connection verified successfully in ${latencyMs}ms! Read, Write & Delete operations zero egress ready.`,
        readOk,
        writeOk,
        deleteOk,
        testedAt: new Date().toISOString(),
        details: {
          ...
          publicUrl: this.config.publicUrl || 'Direct R2 URL',
        },
      };
    } catch (err: any) {
      return {
