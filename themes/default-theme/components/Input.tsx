    ...
    return (
      <div className={`w-full space-y-1.5 ${containerClassName}`}>
        {label && (
          <label
            htmlFor={inputId}
            className="block text-xs font-bold text-[#18181B] dark:text-[#FDF2F8] transition-colors"
          >
            {label}
          </label>
        )}
        ...
        <div className="relative flex items-center">
          {leftIcon && (
            <div className="absolute left-3.5 text-[#A1A1AA] dark:text-[#8E7890] flex items-center pointer-events-none shrink-0">
              {leftIcon}
            </div>
          )}
          ...
          <input
            ...
            onChange={onChange}
            disabled={disabled}
            className={`w-full bg-[#FFF9FC] dark:bg-[#241A30] text-[#18181B] dark:text-[#FDF2F8] placeholder-[#A1A1AA] dark:placeholder-[#8E7890] text-xs font-medium rounded-2xl border transition-all duration-200 py-2.5 px-3.5 outline-none ${
              leftIcon ? 'pl-10' : ''
          ...
          <div className="absolute right-3 flex items-center gap-1.5 text-[#A1A1AA] dark:text-[#8E7890]">
            {clearable && value && !disabled && (
              <button
                type="button"
                onClick={onClear}
                className="p-1 hover:text-[#18181B] dark:hover:text-[#FDF2F8] rounded-md transition-colors"
                tabIndex={-1}
              >
                <X size={14} />
              </button>
